...
function Comment(props) {
  ...
  const handleSend = () => {
    ...
    axios.post(`${process.env.REACT_APP_API}/detail/cmt`, {
      idProduct: props.id,
      cmt: cmt,
      name: name,
      day: day,
  ...
  useEffect(() => {
    async function getData() {
      const res = await axios.get(`${process.env.REACT_APP_API}/comment`);
      return res;
    }
  ...
  const deleteCMT = (id, username) => {
    ...
    if (name == 'admin') {
      if (window.confirm('Bạn muốn xóa bình luận này không ? ')) {
        ...
        axios.get(`${process.env.REACT_APP_API}/delete/cmt/${id}`)
