...
function AdmProduct() {
  ...
  useEffect(() => {
    async function getData() {
      const res = await axios.get(`${process.env.REACT_APP_API}/api/listpet`);
      return res;
    }
  ...
  const handleAddProduct = () => {
    ...
    axios.post(`${process.env.REACT_APP_API}/admin/addproduct`, {
      name: name,
      price: price,
      image: image,
      detail: detail
  ...
  const handleAddEdit = () => {
    axios.post(`${process.env.REACT_APP_API}/edit/product`, {
      name: name,
      price: price,
      image: image,
      detail: detail,
  ...
  const handleDelete = (id) => {
    if (window.confirm('Bạn muốn xóa Sản Phẩm này không ? ')) {
      ...
      axios.get(`${process.env.REACT_APP_API}/delete/product/${id}`)
